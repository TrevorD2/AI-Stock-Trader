...
class Value_Agent(Model):
    ...
    # Get predicted value for being in state x
    def call(self, x):
        d1 = self.d1(x)
        d2 = self.d2(d1)
        lstm = self.lstm(d2)
...
class Actor_Critic(Model):
    def __init__(self,  number_of_stocks: int, epsilon: float, epsilon_decay: float, min_epsilon: float, noise_stdev: float, noise_decay: float, min_noise: float, lra: float, lrc: float, clip_param: float = 0.2):
        super().__init__()
        self.q_agent = Q_Agent(noise_stdev=noise_stdev, noise_decay=noise_decay, min_noise=min_noise)
    ...
    def learn(self, states, actions, advantages, old_probs, discount_rewards):
        ...
        # Get the trainable variables for the actor (both action and quantity agents)
